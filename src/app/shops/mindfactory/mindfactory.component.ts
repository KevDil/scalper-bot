...
import { URLS } from '../../constants/mindfactory-constants';

@Component({
  selector: 'app-mindfactory',
  imports: [CommonModule],
  templateUrl: './mindfactory.component.html',
  styleUrl: './mindfactory.component.scss',
})
export class MindfactoryComponent implements OnInit, OnDestroy {
  ...
  async fetchData(): Promise<void> {
    try {
      const response = await fetch('/api/mindfactory');
      if (!response.ok) {
      ...
      data.forEach((item: { name: string; available: boolean }) => {
        ...
        if (changed) {
          console.log(`Mindfactory: ${item.name} verfügbar:`, item.available);
