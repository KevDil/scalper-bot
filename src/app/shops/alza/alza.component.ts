...
import { URLS } from '../../constants/alza-constants';

@Component({
  selector: 'app-alza',
  imports: [CommonModule],
  templateUrl: './alza.component.html',
  styleUrl: './alza.component.scss',
})
export class AlzaComponent implements OnInit, OnDestroy {
  ...
  async fetchData(): Promise<void> {
    try {
      ...
      data.forEach((item: { name: string; available: boolean }) => {
        ...
        if (changed) {
          console.log(`Alza: ${item.name} verfügbar:`, item.available);
